...
ERNO.Cubelet = function (cube, id, colors) {
  ...
  //  Our Cube can directly address its Cubelet children,only fair the Cubelet can address their parent Cube!
  //我们的Cube可以直接称呼其Cubelet的孩子，只有公平的Cubelet才能称呼其父母Cube！

  this.cube = cube;

  //  Our Cubelet's ID is its unique number on the Cube. 我们的Cubelet的ID是它在Cube上的唯一编号。
  //  Each Cube has Cubletes numbered 0 through 26. 每个立方体都有编号为0到26的立方体。
  //  Even if we're debugging (and not attached to an actual Cube) 即使我们正在调试（并且没有连接到实际的多维数据集）
  //  we need an ID number for later below 我们稍后需要一个身份证号码
  //  when we derive positions and rotations for the Cubelet faces. 然后我们导出Cubelet面的位置和旋转。
  ...
  let code="";
  for (var i = 0; i < 6; i++) {
    //  Before we create our face's THREE object we need to know where it should be positioned and rotated.(This is based on our above positions and rotations map.)
    //在我们创建面部的THREE对象之前，我们需要知道它应该定位和旋转的位置。（这是基于我们上面的位置和旋转地图。）
    ...
    //  and also allow text to be placed on it.
      ...
      if (id == 0) {
        ...
        this.faces[i].code=id + "-" + i //code;
      }
      this.faces[i].code=id + "--" + i
