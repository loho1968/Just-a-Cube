/*
	...
	立方体可以通过其面进行检查（有关面与切片的更多信息，请参见切片）。从浏览器的JavaScript控制台：
	  this.inspect()
	...
	这将使用Face的紧凑检查模式显示每个Face的Cubelet索引和颜色。可以通过传递一个非false值作为参数来访问非紧凑模式：
	  this.inspect( true )
...
ERNO.Cube = function (parameters) {
	...
	//	The textureSize sets the physical size of the cublets in pixels. textureSize以像素为单位设置小立方体的物理大小。
	//	This is useful for rendering purposes as browsers don't downsample textures very well, nor is upsamlping 这对于渲染很有用，因为浏览器不能很好地对纹理进行下采样，也不能进行上采样
	...
	//  Size matters? Cubelets will attempt to read these values.尺寸很重要？Cubelets将尝试读取这些值。
	...
	//  If we enable Auto-Rotate then the cube will spin (not twist!) in space by adding the following values to the Three object on each frame.
	...
	//  We're going to map that front face from left to right (3), and top to bottom (3):that's 3 x 3 = 9 Cubelets.
	//  But then behind the Front slice we also have a Standing slice (9) and Back slice (9),so that's going to be 27 Cubelets in total to create a Cube.
	...
	([
		...
		[Y, O, , , B,], [Y, O, , , ,], [Y, O, G, , ,],//   0,  1,  2
		[Y, , , , B,], [Y, , , , ,], [Y, , G, , ,],//   3,  4,  5
		[Y, , , R, B,], [Y, , , R, ,], [Y, , G, R, ,],//   6,  7,  8
		...
		[ , O, , , B, W], [, O, , , , W], [, O, G, , , W],//  18, 19, 20
		[ , , , , B, W], [, , , , , W], [, , G, , , W],//  21, 22, 23
		[ , , , R, B, W], [, , , R, , W], [, , G, R, , W] //  24, 25, 26



	]).forEach(function (cubeletColorMap, cubeletId) {
    // console.log(cubeletId,cubeletColorMap)
    //todo:生成cubelet
    let tmp=new ERNO.Cubelet(this, cubeletId, cubeletColorMap)
    //todo:是否显示Code
    tmp.showCode= parameters.showCode === undefined ? false : parameters.showCode;
    console.log(tmp.showCode)
    this.cubelets.push(tmp);
...
ERNO.extend(ERNO.Cube.prototype, {
	...
	PRESERVE_LOGO: 'RrLlUuDdFfBbSsMmEe',             //  Preserve the logo position and rotation.
